document.addEventListener('DOMContentLoaded', () => {
  ...
  if (userGrowthCanvas && revenueGrowthCanvas && donutCanvas) {
    ...
    const adminStyle  = getComputedStyle(document.documentElement);
    const A_PINK       = adminStyle.getPropertyValue('--pink').trim();
    const A_PINK_LIGHT = adminStyle.getPropertyValue('--pink-light').trim();
    const A_YELLOW     = adminStyle.getPropertyValue('--yellow').trim();
    const A_TEAL       = adminStyle.getPropertyValue('--teal').trim();
    const A_SURFACE    = adminStyle.getPropertyValue('--surface').trim();
    const A_MUTED      = adminStyle.getPropertyValue('--text-muted').trim();

    const adminMonths  = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
    const userData     = [36000, 42000, 45000, 46500, 47500, 50000];
    const adminRevenueData = [175, 210, 235, 255, 280, 0]; // Jun has no data yet

    // ---- Helpers ----
    function adminSetupCanvas(canvas) {
      const wrapper  = canvas.parentElement;
      canvas.width   = wrapper.offsetWidth;
      canvas.height  = wrapper.offsetHeight;
      return canvas.getContext('2d');
    ...
    function adminShowTooltip(tooltip, x, y, month, value, valueColor) {
      tooltip.innerHTML = `${month}<span style="color:${valueColor}">${value}</span>`;
      tooltip.classList.remove('hidden');
      tooltip.style.left = (x + 14) + 'px';
      tooltip.style.top  = (y - 10) + 'px';
    ...
    function adminHideTooltip(tooltip) {
      tooltip.classList.add('hidden');
    }
    ...
    function drawUserGrowthChart() {
      ...
      const ctx     = adminSetupCanvas(canvas);
      ...
      function xPos(i)   { return PAD.left + (i / (adminMonths.length - 1)) * chartW; }
      function yPos(val) { return PAD.top  + chartH - (val / maxVal) * chartH; }
      ...
      function draw(hoverIdx = -1) {
        ...
        // X labels
        ctx.textAlign = 'center';
        adminMonths.forEach((m, i) => ctx.fillText(m, xPos(i), H - PAD.bottom + 18));
        ...
        userData.forEach((v, i) => {
          ...
          ctx.beginPath();
          ctx.arc(xPos(i), yPos(v), isHover ? 7 : 5, 0, Math.PI * 2);
          ctx.fillStyle   = isHover ? A_PINK_LIGHT : A_PINK;
          ctx.strokeStyle = A_SURFACE;
          ctx.lineWidth   = 2;
          ctx.fill();
          ctx.stroke();
        });
      ...
      canvas.addEventListener('mousemove', (e) => {
        ...
        if (closest >= 0) {
          ...
          adminHideTooltip(tooltip);
        }
      });

      canvas.addEventListener('mouseleave', () => { draw(); adminHideTooltip(tooltip); });
    }
    ...
    function drawRevenueGrowthChart() {
      ...
      const barW   = (chartW / adminMonths.length) * 0.55;

      function xCenter(i) { return PAD.left + (i + 0.5) * (chartW / adminMonths.length); }
      function yPos(val)  { return PAD.top  + chartH - (val / maxVal) * chartH; }
      ...
      function draw(hoverIdx = -1) {
        ...
        // X labels
        ctx.textAlign = 'center';
        adminMonths.forEach((m, i) => ctx.fillText(m, xCenter(i), H - PAD.bottom + 18));

        // Bars — skip Jun (no data)
        adminRevenueData.forEach((v, i) => {
          if (v === 0) return;
          const x    = xCenter(i) - barW / 2;
          const y    = yPos(v);
          const bH   = chartH - (y - PAD.top);
      ...
      canvas.addEventListener('mousemove', (e) => {
        ...
        let closest = -1, minDist = (chartW / adminMonths.length) / 2;
        adminRevenueData.forEach((v, i) => {
          if (v === 0) return;
          const d = Math.abs(mouseX - xCenter(i));
          if (d < minDist) { minDist = d; closest = i; }
        ...
        if (closest >= 0) {
          adminShowTooltip(
            ...
            yPos(adminRevenueData[closest]) / scaleY,
            adminMonths[closest],
            `revenue : $${adminRevenueData[closest]}K`,
            A_TEAL
          );
        } else {
          adminHideTooltip(tooltip);
    ...
    function drawDonutChart() {
      ...
      const slices = [
        { label: 'Web Dev',      value: 30, color: A_PINK },
        { label: 'Data Science', value: 25, color: A_YELLOW },
        { label: 'Mobile',       value: 20, color: A_TEAL },
